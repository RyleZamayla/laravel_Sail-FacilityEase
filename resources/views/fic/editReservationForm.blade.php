        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="w-auto rounded-lg bg-white">
                ...
                <div class="w-full py-6 px-4">
                    <form
                        action="@if (Auth::user()->user_role->where('roleID', 1)->count() > 0) {{ route('updateReservationDetailsPending', ['id' => $reservation->id]) }}
                @elseif (Auth::user()->user_role->where('roleID', 2)->count() > 0)
                  {{ route('fic.updateReservationDetailsPending', ['id' => $reservation->id]) }}
                @elseif (Auth::user()->user_role->where('roleID', 3)->count() > 0)
                  {{ route('user.updateReservationDetailsPending', ['id' => $reservation->id]) }}
                @elseif (Auth::user()->user_role->where('roleID', 4)->count() > 0)
                  {{ route('user.updateReservationDetailsPending', ['id' => $reservation->id]) }}
                @elseif (Auth::user()->user_role->where('roleID', 5)->count() > 0)
                  {{ route('user.updateReservationDetailsPending', ['id' => $reservation->id]) }}
                @elseif (Auth::user()->user_role->where('roleID', 6)->count() > 0)
                  {{ route('user.updateReservationDetailsPending', ['id' => $reservation->id]) }} @endif"
                        ...
                        <div class="w-full px-3">
                            <div class="">
                                ...
                                <input type="text" name="facilityID" value="{{ $reservation->facility->facility }}"
                                    class="mt-2 w-full rounded-md border border-[#e0e0e0] bg-facilityEaseLightGrey py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md cursor-not-allowed"
                                    readonly />
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap mt-10">
                            ...
                            <div class="w-full px-3 sm:w-1/2">
                                <div class="">
                                    ...
                                    <input type="number" name="noOfdays" id="days" min="1"
                                        placeholder="(e.g. 1,2,3)" value="{{ old('noOfdays', $reservation->noOfdays) }}"
                                        class="mt-2 w-full rounded-md border border-[#e0e0e0] bg-facilityEaseLightGrey py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md cursor-not-allowed"
                                        readonly />
                                </div>
                            </div>
                        ...
                        <div class ="mt-10 px-3">
                            <h1 class="text-lg font-medium">Equipment | toggle checkbox when not needed:
                                <span class="text-red-500">*</span>
                            </h1>
                            <div class="pl-5">
